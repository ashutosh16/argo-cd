...
import {
    Application,
    ApplicationTree, HealthStatusCode,
    HostResourceInfo,
    InfoItem,
    Node,
    Pod,
...
export class PodView extends React.Component<PodViewProps> {
    ...
    private switchToCountView(appDetails: AppDetailsPreferences) {
        services.viewPreferences.updatePreferences({
            appDetails: {
                ...
                podView: {
                    ...appDetails.podView,
                    showPodHealthIcons: !appDetails.podView.showPodHealthIcons
                }
            }
        });
        {console.log(appDetails.podView.showPodHealthIcons)}
    }
                                ...
                                {podPrefs.sortMode === 'node' && (
                                    <div className='pod-view__settings__section'>
                                        <button
                                            ...
                                            <i className={`fa fa-${podPrefs.hideUnschedulable ? 'eye-slash' : 'eye'}`}
                                        ...
                                        return (
                                            <div
                                                ...
                                                <div
                                                    ...
                                                    <div style={{display: 'flex', alignItems: 'center'}}>
                                                        <div style={{marginRight: '10px'}}>
                                                            ...
                                                            <br/>
                                                            {<div
                                                                style={{textAlign: 'center'}}>{ResourceLabel({kind: group.kind})}</div>}
                                                        </div>
                                                        <div style={{lineHeight: '15px'}}>
                                                            <b style={{wordWrap: 'break-word'}}>{group.name || 'Unknown'}</b>
                                                            {group.resourceStatus && (
                                                                <div>
                                                                    {group.resourceStatus.health && <HealthStatusIcon
                                                                        state={group.resourceStatus.health}/>}
                                                                    &nbsp;
                                                                    {group.resourceStatus.status &&
                                                                        <ComparisonStatusIcon
                                                                            status={group.resourceStatus.status}/>}
                                                                </div>
                                                            )}
                                                        </div>
                                                ...
                                                <div className='pod-view__node__container'>
                                                    ...
                                                    <div
                                                            className='pod-view__node__container pod-view__node__container--stats'>
                                                    ...
                                                    <div className='pod-view__node__pod-container pod-view__node__container'>
                                                        <div className='pod-view__node__pod-container__pods'>
                                                            {this.renderPodsViews(group.pods.filter(pod => pod.health == "Healthy"),  podPrefs.showPodHealthIcons)}
                                                        </div>
                                                        <div className='pod-view__node__pod-container__pods'>
                                                            {this.renderPodsViews(group.pods.filter(pod => pod.health == "Degraded"),   podPrefs.showPodHealthIcons)}
                                                        </div>
                                                        <div className='pod-view__node__pod-container__pods'>
                                                            {this.renderPodsViews(group.pods.filter(pod => pod.health == "Progressing"),  podPrefs.showPodHealthIcons)}
                                                        </div>
                                                        {(
    ...
    renderPodsViews(pods: Pod[], showPodHealthIcons: boolean) {
        ...
        return (
            (pods.length != 0 && !showPodHealthIcons)? (
                <span>
                    ...
                    <div className='pod-view__node__label--large'>{pods.length} {pods[0].health} pods </div>
                 </span>
                ) : (
                    pods.sort(sortYoungerPods).map(pod =>( <DropDownMenu
                            key={pod.uid}
                            anchor={() => (
                                <Tooltip
                                    content={
